...
[MCP](https://modelcontextprotocol.io) server exposing the PricingDay SDK as
two agent tools — `pricing-day_list` and `pricing-day_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pricing-day-mcp)
make build

# 2. Provide credentials via the environment
export PRICING_DAY_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user pricing-day \
  -- /absolute/path/to/pricing-day-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./pricing-day-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// pricing-day_list: first page of records
{ "entity": "catalog" }
{ "entity": "catalog", "query": { } }

// pricing-day_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pricing-day-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user pricing-day \
     -- "$PWD"/dist/*/pricing-day-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `pricing-day_list` and `pricing-day_load` tools now appear
   in new sessions. Ask the agent to *"list catalog using pricing-day"*
   and it calls `pricing-day_list` with `{"entity":"catalog"}`.
...
```sh
export PRICING_DAY_APIKEY=sk_live_xxx            # API key
export PRICING_DAY_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `pricing-day_list` tool
...
### Call the `pricing-day_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `pricing-day_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `pricing-day_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PRICING_DAY_APIKEY` | API key sent with every request. |
| `PRICING_DAY_BASE` | Optional override of the API base URL. |
...
```sh
./pricing-day-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"pricing-day_load","arguments":{"entity":"catalog","query":{"id":1}}}}'
```
