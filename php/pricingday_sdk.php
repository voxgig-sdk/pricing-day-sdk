...
class PricingDaySDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = PricingDayHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = PricingDayHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!PricingDayFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, PricingDayFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return PricingDayUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = PricingDayHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = PricingDayHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = PricingDayHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new PricingDayError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = PricingDayHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new PricingDayError("graphql_error",
                "PricingDaySDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
