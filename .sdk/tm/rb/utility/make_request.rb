...
module PricingDayUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = PricingDayResponse.new({})
    ...
    if fetch_err
      ...
      response = PricingDayResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PricingDayResponse.new(fetched)
    else
