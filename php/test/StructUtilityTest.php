...
// PricingDay SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = PricingDayOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(PricingDayOmni::structfix($now, Struct::undef()))
                        !== json_encode(PricingDayOmni::structfix($val, Struct::undef()))) {
