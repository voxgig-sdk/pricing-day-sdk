...
class CatalogEntity
  ...
  def data_set(args)
    if args
      @_data = PricingDayHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = PricingDayHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [CatalogLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Catalog.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Catalog, Hash] the loaded Catalog; raises PricingDayError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = PricingDayHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
