...
Pricing Day API client, generated from the OpenAPI spec.

This API provides data regarding the price changes of popular products in Russia, impacted by the ongoing conflict initiated by Putin against Ukraine.

Learn more about Pricing Day API at [github.com/l0v3m0n3y/pricingday](https://github.com/l0v3m0n3y/pricingday).

This is an unofficial SDK for the Pricing Day public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
// Load catalog data (returns a Catalog)
const catalog = await client.Catalog().load()
console.log(catalog)
```
...
```bash
cd go-mcp && go build -o pricing-day-mcp .
```
...
```json
{
  "mcpServers": {
    "pricing-day": {
      "command": "/abs/path/to/pricing-day-mcp"
    }
  }
}
```
...
| --- | --- | --- |
| **Catalog** | The Catalog entity (load). | `/_next/data/ltZLXFvzTh1RillBuqeGx/index.json` |
...
# Load a specific catalog (returns the record, raises on error)
catalog = client.Catalog().load()
print(catalog)
```
...
```go
import sdk "github.com/voxgig-sdk/pricing-day-sdk/go"
...
Generated from the Pricing Day API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
