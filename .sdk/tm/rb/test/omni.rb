...
module PricingDayOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if PricingDayOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = PricingDayOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| PricingDayOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || PricingDayOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = PricingDaySpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = PricingDayError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { PricingDayOmni.errify(err) }
