...
class CatalogEntity
{
    ...
    {
        if ($args) {
            $this->_data = PricingDayHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = PricingDayHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *
     * @param CatalogLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CatalogLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Catalog|array The loaded Catalog as an assoc-array at the
     *   SDK boundary; throws PricingDayError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = PricingDayHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
