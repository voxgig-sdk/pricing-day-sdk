...
AQL-driven command-line client **and** interactive REPL for the PricingDay
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pricing-day-cli)
make build

# 2. See usage (words, entities, env vars)
./pricing-day-cli --help
...
./pricing-day-cli load 1 catalog            # {id:1} shorthand
./pricing-day-cli load '{id:1}' catalog       # explicit match map

# 5. Override the API base URL for a single call
PRICING_DAY_BASE=https://api.example.com ./pricing-day-cli load 1 catalog

# 6. No arguments -> interactive REPL
./pricing-day-cli
pricing-day> load 1 catalog
pricing-day> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pricing-day-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/pricing-day-cli load 1 catalog
   ```
...
```sh
./pricing-day-cli load 1 catalog          # scalar shorthand for {id:1}
./pricing-day-cli load '{id:1}' catalog     # explicit match map
```
...
./pricing-day-cli load 1 catalog
```
...
Run with no arguments to open a REPL (prompt `pricing-day>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `catalog`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/pricing-day-cli`. |
...
(`./pricing-day-cli <expr>`) and interactively (the REPL), and expressions compose the
