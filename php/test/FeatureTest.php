...
class FtCtrl extends PricingDayControl
{
...
class FtClient
{
    ...
    private PricingDayUtility $utility;

    public function __construct(PricingDayUtility $utility, array $options)
    {
    ...
    public function get_utility(): PricingDayUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PricingDayContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PricingDayContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PricingDayContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PricingDayConfig::make_feature($name);
    ...
    private static function buildUrl(PricingDaySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PricingDayError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PricingDayResponse($fetched);
        }
    ...
    private function fail(PricingDayContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PricingDayContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
