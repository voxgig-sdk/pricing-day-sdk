...
module PricingDayUtilities
  MakePoint = ->(ctx) {
    if ctx.out["point"]
      ...
      return nil, preset if preset.is_a?(PricingDayError)
      ctx.point = preset
    ...
    if op.points.length == 1
      ...
      matched = false
      op.points.each do |p|
        select_def = PricingDayHelpers.to_map(VoxgigStruct.getprop(p, "select"))
        found = true
      ...
      if reqselector
        req_action = VoxgigStruct.getprop(reqselector, "$action")
        if req_action && point
          point_select = PricingDayHelpers.to_map(VoxgigStruct.getprop(point, "select"))
