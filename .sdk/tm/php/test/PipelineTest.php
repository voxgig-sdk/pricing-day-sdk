...
{
    private static function utility(): PricingDayUtility
    {
        return new PricingDayUtility();
    }
    ...
    private static function ctx(array $over = []): PricingDayContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new PricingDayContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new PricingDayOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof PricingDayError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new PricingDayOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = PricingDayMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new PricingDayOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = PricingDayMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new PricingDayOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = PricingDayMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PricingDaySpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = PricingDayMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new PricingDayResponse([]),
            'result' => new PricingDayResult([]),
        ]);
        [, $err] = PricingDayMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new PricingDaySpec([]),
            'response' => null,
            'result' => new PricingDayResult([]),
        ]);
        [, $err] = PricingDayMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new PricingDaySpec(['step' => 's']),
            'response' => new PricingDayResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new PricingDayResult([]),
        ]);
        [, $err] = PricingDayMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new PricingDaySpec(['step' => 's']),
            'response' => new PricingDayResponse(self::resp(200, ['v' => 1])),
            'result' => new PricingDayResult([]),
        ]);
        [, $err] = PricingDayMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new PricingDaySpec(['step' => 's']),
            'response' => new PricingDayResponse(self::resp(200, ['v' => 2])),
            'result' => new PricingDayResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        PricingDayMakeResponse::call($ctx);
    ...
    {
        $preset = new PricingDayResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new PricingDaySpec([]),
            'response' => new PricingDayResponse([]),
            'result' => new PricingDayResult([]),
        ...
        [$got, $err] = PricingDayMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new PricingDayResult([])]);
        [, $err] = PricingDayMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new PricingDaySpec([]), 'result' => null]);
        [, $err] = PricingDayMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PricingDaySpec(['step' => 's']),
            'result' => new PricingDayResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new PricingDayOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PricingDayMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PricingDaySpec(['step' => 's']),
            'result' => new PricingDayResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new PricingDayOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PricingDayMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PricingDayResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new PricingDaySpec([]),
            'result' => new PricingDayResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = PricingDayMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PricingDayMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new PricingDayError('boom', 'boom');
        $utility->fetcher = function (PricingDayContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PricingDaySpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = PricingDayMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (PricingDayContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = PricingDayMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(PricingDayResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        PricingDayMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (PricingDayContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PricingDaySpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = PricingDayMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new PricingDayResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new PricingDaySpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = PricingDayMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = PricingDayMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = PricingDayMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new PricingDayResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, PricingDayDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new PricingDayResult(['ok' => false])]);
        $this->expectException(PricingDayError::class);
        PricingDayDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new PricingDayResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', PricingDayMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        PricingDayMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new PricingDayBaseFeature();
        $b = new PricingDayBaseFeature();
        PricingDayFeatureAdd::call($ctx, $a);
        PricingDayFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): PricingDayBaseFeature
    {
        $f = new PricingDayBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        PricingDayFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): PricingDayContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new PricingDaySpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PricingDayPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        PricingDayPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new PricingDayResponse(['headers' => null]),
            'result' => new PricingDayResult([]),
        ]);
        PricingDayResultHeaders::call($ctx);
