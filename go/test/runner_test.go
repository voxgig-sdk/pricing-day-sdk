...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("PRICING_DAY_TEST_LIVE") == "TRUE" ||
		os.Getenv("PRICING_DAY_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("PRICING_DAY_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.PricingDaySDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.PricingDayError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.PricingDaySDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.PricingDayError{Msg: msg, Code: code}
}
