...
{
    public function test_direct_load_catalog(): void
    {
        $setup = catalog_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-catalog", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
...
function catalog_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new PricingDaySDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
