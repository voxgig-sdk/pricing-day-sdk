...
const TEST_SEED = { "entity": { "catalog": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+PricingDaySDK\b/.test(code) ||
        /\bPricingDaySDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+PricingDaySDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/PricingDaySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
