...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = PricingDayOmni.make_runner(TEST_JSON_FILE, PricingDaySDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    PricingDayError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : PricingDayOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = PricingDayOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
