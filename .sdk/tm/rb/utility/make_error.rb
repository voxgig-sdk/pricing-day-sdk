...
module PricingDayUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = PricingDayContext.new({}, nil)
    end
    op = ctx.op || PricingDayOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || PricingDayResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(PricingDayError) ? err.msg : err.to_s
    ...
    sdk_err = PricingDayError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(PricingDayError)
