// Command PricingDay-cli is a CLI + REPL for the PricingDay SDK,
...
const prompt = "pricing-day"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "catalog"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      PRICING_DAY_APIKEY (api key), PRICING_DAY_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: PRICING_DAY_APIKEY carries the API key and
	// PRICING_DAY_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("PRICING_DAY_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("PRICING_DAY_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewPricingDaySDK(opts)
