...
import (
	...
	omni "github.com/voxgig-sdk/pricing-day-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PricingDayError); is {
				out := map[string]any{
					"name":    "PricingDayError",
...
func omniCtx(arg any, client *sdk.PricingDaySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PricingDaySDK); is {
			client = live
