...
//
// NewPricingDaySDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.PricingDaySDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewPricingDaySDK(opts)
}
...
func fcCandidates(client *sdk.PricingDaySDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.PricingDaySDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.PricingDaySDK, name string) any {
