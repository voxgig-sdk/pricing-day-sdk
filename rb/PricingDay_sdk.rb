...
require_relative 'PricingDay_types'
...
class PricingDaySDK
  ...
  def initialize(options = {})
    ...
    feature_opts = PricingDayHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = PricingDayHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, PricingDayFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    PricingDayUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = PricingDayHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = PricingDayHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = PricingDayHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => PricingDayError.new(
        "#{op}_allow",
        "PricingDaySDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = PricingDayHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = PricingDayError.new(
        "graphql_error", "PricingDaySDK: graphql: #{msg}")
      res["graphql"] = errors
    end
