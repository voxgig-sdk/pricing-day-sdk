...
// PricingDay SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class PricingDayMakePoint
{
    public static function call(PricingDayContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof PricingDayError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = PricingDayHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = PricingDayHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
