...
from pricingday_sdk.core.helpers import get_ctx_prop, to_map
...
class PricingDayContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, PricingDaySpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, PricingDayResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, PricingDayResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return PricingDayError(code, msg, self)
