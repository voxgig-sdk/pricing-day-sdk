...
class CatalogEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set PRICINGDAY_TEST_CATALOG_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def catalog_basic_setup(extra)
  ...
  entid_env_raw = ENV["PRICINGDAY_TEST_CATALOG_ENTID"]
  ...
  if env["PRICINGDAY_TEST_LIVE"] == "TRUE"
    ...
    client = PricingDaySDK.new(Helpers.to_map(merged_opts))
  end
