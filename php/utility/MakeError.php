...
// PricingDay SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class PricingDayMakeError
{
    public static function call(?PricingDayContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new PricingDayContext([], null);
        }
        $op = $ctx->op ?? new PricingDayOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new PricingDayResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof PricingDayError) ? $err->msg : (string)$err;
        $msg = "PricingDaySDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new PricingDayError('', $msg, $ctx);
        ...
        if ($err instanceof PricingDayError) {
